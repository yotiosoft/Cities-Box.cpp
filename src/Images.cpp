...
ImagesStruct::ImagesStruct()
    : m_catalog(rust::images::new_catalog()) // Rust側のオブジェクトを初期化
{}

void ImagesStruct::load(const string filePath, const string tag) {
    // 透明色指定なしの場合は、無効な色を渡して呼び出し
    load(filePath, tag, Color(-1, -1, -1));
}

void ImagesStruct::load(const string filePath, const string tag, const Color transparent) {
    // 1. Rustにディレクトリをスキャンさせる
    // String(Siv3D) -> std::string -> char* 経由でRustへ
    m_catalog->scan_directory(filePath);
    ...
    for (const auto& entry : entries) {
        ...
        FilePath s3dPath = Unicode::FromUTF8(static_cast<std::string>(entry.full_path));
        ...
        newImage.file_name = static_cast<std::string>(entry.file_name);
        newImage.tag = tag;

        // entry.stem はファイル名（拡張子なし）
        std::string key_stem = static_cast<std::string>(entry.stem);
        images[tag][key_stem] = newImage;
    }
}
