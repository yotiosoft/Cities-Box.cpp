...
void DetailsBar::printTemperature(int temperature) {
	...
	(*font)(Format(temperature)+U"Åé").draw(weatherRegion+30+1, position.y+1, ColorF(0.0, 0.5));
	(*font)(Format(temperature)+U"Åé").draw(weatherRegion+30, position.y, Color(240));
	
	temperatureRegion = weatherRegion+30+(*font)(Format(temperature)+U"Åé").region().w;
}
...
void DetailsBar::printMoney(int money) {
	(*font)(U"Åò "+Format(money)).draw(populationPrintRegion+10+1, position.y+1, ColorF(0.0, 0.5));
	(*font)(U"Åò "+Format(money)).draw(populationPrintRegion+10, position.y, Color(240));
	
	moneyPrintRegion = populationPrintRegion+10+(*font)(U"Åò "+Format(money)).region().w;
}
