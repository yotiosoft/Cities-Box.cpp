...
void Menu::set(PositionStruct new_position, SizeStruct new_size, Font* new_font8, Font* new_font16) {
	...
	roadButton.set(IconFont::Road, 30, 26, PositionStruct{ 0, 6 });
	railRoadButton.set(IconFont::Railroad, 30, 28, PositionStruct{ 2, 2 });
...
void Menu::draw(RCOIFstruct demand, int population_count, int money) {
	...
	roadButton.put(PositionStruct{ position.x + 55, position.y });
	railRoadButton.put(PositionStruct{ position.x + 55+35, position.y });
	residentialButton.put(PositionStruct{position.x+55+70, position.y});
	commercialButton.put(PositionStruct{position.x+55+105, position.y});
	industrialButton.put(PositionStruct{position.x+55+140, position.y});
	farmButton.put(PositionStruct{position.x+55+175, position.y});
	publicButton.put(PositionStruct{position.x+55+210, position.y});
	
	parkButton.put(PositionStruct{position.x+310, position.y});
	waterWayButton.put(PositionStruct{position.x+310+35, position.y});
	airPortButton.put(PositionStruct{position.x+310+70, position.y});
	fieldButton.put(PositionStruct{position.x+310+105, position.y});
	
	informationButton.put(PositionStruct{position.x+460, position.y});
	rateButton.put(PositionStruct{position.x+460+35, position.y});
	deleteButton.put(PositionStruct{position.x+460+70, position.y});
	budgetButton.put(PositionStruct{position.x+460+105, position.y});
	settingButton.put(PositionStruct{position.x+460+140, position.y});
	saveButton.put(PositionStruct{position.x+460+175, position.y});
